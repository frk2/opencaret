    ...
    def loop(self):
        while not rospy.is_shutdown():
            ...
            if ret:
                ...
                self.last_centroids = self.calculate_centroids(perspective, average=average)


                # Fit poly to different axis as its more stable, then flip axes
                self.last_fit = np.poly1d(np.polyfit(self.last_centroids[:, 0],
                ...
                road_message.cte = self.last_ctes[-25]
                road_message.curvature = self.last_curvatures[-25]
                self.road_surface_pub.publish(road_message)
                self.cte_pub.publish(Float32(data=self.last_ctes[-25]))
                self.curvature_pub.publish(Float32(data=self.last_curvatures[-25]))
