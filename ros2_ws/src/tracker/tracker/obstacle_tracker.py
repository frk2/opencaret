...
class ObstacleTracker(Node):
    ...
    def on_radar_tracks(self, tracks_msg):
        ...
        for track in tracks_msg.radar_tracks:
            if track.valid_count >= ObstacleTracker.VALIDITY_THRESH and track.lng_dist > 0.0 and \
                -half_width < track.lat_dist < half_width:
                points_list.append([track.lng_dist, track.lat_dist])
                usable_radar_tracks.append(track)

        self.all_obstacles = []
        if len(points_list) > 0:
            dbscan = DBSCAN(eps=4.0, min_samples=1).fit(points_list)
    ...
    def calculate_and_publish_lead(self):
        ...
        for obstacle in self.all_obstacles:
            if closest_obs_in_range is None:
                closest_obs_in_range = obstacle
            elif obstacle.point.x < closest_obs_in_range.point.x:
                closest_obs_in_range = obstacle
        ...
        if closest_obs_in_range:
            ...
            if not self.last_lead_vehicle:
                ...
                # self.get_logger().info("update with dist: {}, vel: {}, outx: {}, outv: {}, t: {}".format(
                #     closest_obs_in_range.point.x,
                #     closest_obs_in_range.relative_speed,
                #     dist,vel, current_ts - self.last_ukf_update))
